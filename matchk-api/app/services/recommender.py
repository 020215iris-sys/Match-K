...
Step 4  필터: 부산만 / auto는 소멸위험 구 쿼터(DECLINING_QUOTA) / 이미 도장 찍은 곳 제외(옵션)
...
async def recommend(db: Session, user_lang: str, rec_type: str = "auto",
    ...
    # Step 1: 후보 풀 — 국문 마스터 기준 부산 관광지 (관심 관광지 API 확보 시 교체 지점)
    # 후보 풀은 관광지 타입(12)만 — 음식점/숙박이 랜드마크 추천에 섞이는 문제 방지 (스냅샷 튜닝)
    ...
    candidates = [c for c in (_to_candidate(i) for i in raw) if c]
    ...
    for c in candidates:
        if c.contentid in stamped:
            continue  # 이미 도장 찍은 곳 제외 (Step 4 옵션)
    ...
    top = apply_declining_quota(scored, limit) if rec_type == "auto" else scored[:limit]
