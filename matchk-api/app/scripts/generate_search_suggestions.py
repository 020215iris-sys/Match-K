...
⚠️ 이 스크립트가 하는 일: TARGET_MONTHS(지금은 8·9·10월) × 언어 4개(ko/en/ja/zh)에 대해
   ...
   그 달 행만 읽는다 — 그래서 8·9·10월치를 오늘 한 번에 다 만들어두면, 9월 1일·10월 1일이
   됐을 때 아무도 아무것도 안 해도 자동으로 그 달 문구가 보이게 된다 (스케줄러 불필요).

   대신 트레이드오프: 9·10월 문구는 "오늘(8월) 시점"에 미리 상상해서 만드는 거라, 그때 가서
...
_LANGS = ["ko", "en", "ja", "zh"]
TARGET_MONTHS = [8, 9, 10]  # 앱 시연/심사 기간(9/21 마감)을 커버하는 범위
